...
// at the top of src/components/dashboard/SuggestedWorkflows.tsx
import React from "react";
import { motion } from "framer-motion";
import { PlayCircle } from "lucide-react";
...
// (Remove the entire local `colorClassMap` definition in lines 18–53)
// The component will now use the shared `colorClassMap` you just imported.
            ...
            <div className="flex items-center space-x-3 mb-2">
              <div
                className={`p-2 rounded-lg group-hover:scale-110 transition-transform ${
                  colorClassMap[template.color]?.bg ||
              ...
              >
                <template.icon
                  size={16}
                  className={`${
                    colorClassMap[template.color]?.text || "text-neutral-600"
                  } ${
                    colorClassMap[template.color]?.darkText ||
