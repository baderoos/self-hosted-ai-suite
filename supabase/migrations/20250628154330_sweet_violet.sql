/*
  # Fix Profiles and Workspace Policies

  1. Changes
    - Add avatar_url column to profiles table if it doesn't exist
    - Fix workspace_members policies to prevent infinite recursion
    - Update workspace policies to avoid self-referential conditions
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'workspace_members' AND policyname = 'Users can view their own workspace memberships'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'workspace_members' AND policyname = 'Users can insert their own workspace memberships'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'workspace_members' AND policyname = 'Users can update their own workspace memberships'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'workspace_members' AND policyname = 'Users can delete their own workspace memberships'
  ) THEN
