...
CREATE POLICY IF NOT EXISTS "Users can update their own profile"
  ...
  FOR UPDATE
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());-- Create function to automatically create profile on user creation
-- Create function to automatically create profile on user creation
...
FOR EACH ROW
EXECUTE FUNCTION set_profiles_updated_at();DROP TRIGGER IF EXISTS create_profile_for_user_trigger ON auth.users;
